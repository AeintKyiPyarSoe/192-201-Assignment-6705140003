...
class Order:
    # has-a Customer, has-many OrderItem
    ...
    def __init__(self, customer, items):
        ...
        for item in items:
            if not isinstance(item, OrderItem):
                raise ValueError("All items must be OrderItem objects.")
    ...
    def tax(self):
        ...
        for item in self.items:
            ...
            total_tax += item.product.tax(line_total)
...
def create_customer(name, tier):
    ...
    customer_class = customer_types[tier]
...
def create_orders():
    ...
    for name, tier, item_data in ORDERS:
